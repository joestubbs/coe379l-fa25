...
.. note:: 
    ...
    Conceptually, we can think of the scaling process as just another "model".
...
Let's try scaling a similar array using both StandardScaler and RobustScaler. Note that 
...
.. note:: 
    ...
    Due to the random nature of the original data, your values will be a little different. 
...
MaxAbs Scaler 
-------------
There is one more scalar we will introduce to handle sparse data. We begin with some motivation. 

Motivation: Sparse Data 
~~~~~~~~~~~~~~~~~~~~~~~

In the case of *sparse data*, i.e., datasets where most of the values are 0, it can still be important 
to put variables on the same scale for all the reasons mentioned at the beginning (e.g., to ensure that 
no single column dominates the cost function). However, the spareness is usually an important part of 
the structure of the data and needs to be preserved. 

In this case, the previous two scalars (for different reasons) are not effective. 
Let's think about what would happen if we applied either Standard Scalar or Robust Scalar to a sparse 
dataset. 

In the case of Standard Scalar, each input :math:`x` is transformed to :math:`\frac{x-mean(X)}{std(X)}.`
Since :math:`mean(X)` will not be 0, this transformation will always take the 0s in the dataset to 
nonzero values, thus destroying the spareness structure. 

On the other hand, let's consider the Robust Scalar. This scalar works by removing the median and then 
scaling based on some percentage range. However, with sparse data, most of the values are 0 and thus the 
median will be 0 as well. Since subtracting 0 does nothing, Robust Scalar will have no effect on 
sparse data. 
...
Thus, the last Scaler we will mention is the ``MaxAbsScaler``, short for "maximum absolute" scaler. 
...
.. code-block:: python3 
    ...
    data.loc[data.Insulin == 0, 'Insulin'] = data.Insulin.median().astype('int')
...
GridSearchCV in our previous lecture (at least for the accuracy metric). 
We'll hard code the 13 value for now, but 
...
.. code-block:: python3 
    ...
    >>> pipeline = make_pipeline(StandardScaler(), KNeighborsClassifier(n_neighbors=13))
...
.. code-block:: python3 
    ...
    >>> print(pipeline.score(X_test, y_test))  # apply scaling on testing data, without leaking training data.    
    0.7532467532467533
...
``pipeline`` object, so we can do things like: 
...
best. But what happened to the accuracy? The accuracy is now 73%. That's worse than what we had 
before! What happened??

Let's take a look at what the model did on the *training* set: 

.. code-block:: python3 

    >>> print("scaling best params: ", gscv2.best_params_)
    >>> accuracy_train = accuracy_score(y_train, gscv2.best_estimator_.predict(X_train))

    >>> print(f'Accuracy of best estimator WITH SCALING on train data is: {accuracy_train}')

    Accuracy of best estimator WITH SCALING on train data is: 1.0

Ah! The accuracy is very high on the training data. This is an example of what?

**Take Home Exercise.** How would you modify the above code to: a) take care of the overfitting? 
b) add additional models to the mix, such as the boosting models we looked at? 
